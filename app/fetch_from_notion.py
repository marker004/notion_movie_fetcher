...
def get_movies_from_notion() -> list:
  ...
  all_titles = [row['properties']['Title']['title'][0]['plain_text'] for row in database_rows]
  ...
  for movie in all_titles:
    ...
    result = next(
      (
        ...
        if strip_all_punctuation(result['title'].lower()) == strip_all_punctuation(movie.lower())
      ),
    None)
...
def get_ids():
  for movie in movies_to_search:
    raw_results = just_watch.search_for_item(query=movie, release_year_from=2022)
    result = next((result for result in raw_results['items'] if result['title'].lower() == movie.lower()), None)
    pp(result)
