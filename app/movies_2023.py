...
from shared_items.utils import (
    reversor,
    pp,
    convert_runtime,
...
def convert_relevant_providers(relevant_streaming_ids: list[dict]) -> list[Provider]:
    ...
    relevant_streaming_ids_short_names: list[str] = [
        x["short_name"] for x in relevant_streaming_ids
    ]
    relevant_provider_results: list[dict] = [
        x
        for x in provider_results
        if x["short_name"] in relevant_streaming_ids_short_names
    ]
...
def viewing_option(
    offer: Offer, movie: Movie, relevant_providers: list[Provider]
) -> ViewingOption:
    streaming_service = next(
        (
            x.clear_name
            for x in relevant_providers
            if x.short_name == offer.package_short_name
        )
    )
    ...
    response: ViewingOption = {
        ...
        "human_text": (
            streaming_service if offer.streaming else f"Rent ${offer.retail_price}"
        ),
        "id": movie.id,
...
@measure_execution
def generate_nice_offers(
    titles_with_ids: list[TitleWithId], relevant_providers: list[Provider]
) -> dict[str, Any]:
        ...
        if movie.offers:
            ...
            for offer in movie.offers:
                if (
                    offer.relevant
                    and offer.watchable
                    and (offer.streaming or offer.rental)
                ):
            ...
            sorted_offers = sorted(
                relevant_offers,
                key=lambda k: (reversor(k["type"]), k["quality"], k["price"]),
            )
    ...
    for idx, (title, info) in enumerate(nice_offers.items()):
        try:
            ...
            props: list[NotionProp] = [
                {"name": "Time", "type": "rich_text", "content": info["runtime"]},
                {"name": "JustWatch ID", "type": "number", "content": info["id"]},
                {"name": "JustWatch URL", "type": "url", "content": info["url"]},
                {"name": "Year", "type": "number", "content": info["year"]},
            ]
